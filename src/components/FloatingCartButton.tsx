        ...
        {isOpen && (
          <div className="absolute bottom-16 sm:bottom-20 right-0 w-[calc(100vw-2rem)] sm:w-80 max-w-sm bg-white rounded-2xl shadow-2xl border border-gray-200 mb-4 max-h-[70vh] sm:max-h-[80vh] overflow-hidden flex flex-col">
            <div className="bg-gradient-to-r from-slate-800 to-slate-900 p-4 sm:p-4 text-white flex-shrink-0">
              ...
              <p className="text-cyan-200 text-sm">{totalItems} items • ${totalPrice.toFixed(2)}</p>
            </div>
                ...
                <div key={item.id} className="p-4 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1 pr-3">
                      <h4 className="font-medium text-slate-800 text-sm leading-5">{item.name}</h4>
                      <p className="text-cyan-600 font-semibold text-sm mt-1">${item.price} each</p>
                    </div>
                    <button
                      onClick={() => removeFromCart(item.id)}
            ...
            <div className="flex-shrink-0 p-4 bg-slate-50 border-t border-slate-100">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="font-bold text-xl text-cyan-600">${totalPrice.toFixed(2)}</span>
              </div>
